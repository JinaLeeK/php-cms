	...
	if(isset($_POST['update'])){
		...
		if(!isset($_GET['obj'])) {
			...
			if ($cur_order > $order) {
				...
				while ($row = fetch_array($query)) {
					if ($cur_order > $row['category_order'] && $row['category_order']>=$order) {
						$ud_query = query("UPDATE categories SET category_order = category_order + 1 WHERE category_id={$row['category_id']}") ;
						confirmQuery($ud_query);
					}
				}
			} else if ($cur_order < $order ) {
				$query = query("SELECT * FROM categories");
				while($row = fetch_array($query) ) {
					if ($row['category_order']>$cur_order && $row['category_order']<=$order) {
						$ud_query = query("UPDATE categories SET category_order = category_order-1 WHERE category_id={$row['category_id']}");
						confirmQuery($ud_query);
					}
				}
			...
			$query = query("UPDATE categories SET category_title='{$category_title}', category_image='{$image}', category_order={$order} WHERE category_id={$category_id}");
...
DELIMETER;
				...
				$category .= <<< DELIMETER
				<a href="posts.php?id={$row['category_id']}">
DELIMETER;
			}
     if (isset($current_cat) && $current_cat==$row['category_id']) {
...
function get_my_posts() {
	...
	if ($count <1) {
		...
		if (isset($_GET['status'])) {
			$page_query = query("SELECT * FROM posts WHERE post_author='{$_SESSION['username']}' AND post_status='{$_GET['status']}' LIMIT {$page_1}, {$per_page}");
		} else {
			$page_query = query("SELECT * FROM posts WHERE post_author='{$_SESSION['username']}' AND NOT post_status='temp' LIMIT {$page_1}, {$per_page}");
		}

		confirmQuery($page_query);

				while ($row = fetch_array($page_query)) {
...
DELIMETER;
	...
	if ($count > 1) {
		...
		for($i=1 ; $i<=$count; $i++) {
			if ($i == $page) {
				if (isset($_GET['status'])) {
					echo "<li class='page-item active'><a class='page-link' href='posts.php?mypost&status={$_GET['status']}&page={$i}'>$i</a></li>";
				} else {
					echo "<li class='page-item active'><a class='page-link' href='posts.php?mypost&page={$i}'>$i</a></li>";
				}
			} else {
				if (isset($_GET['status'])) {
				echo "<li class='page-item'><a class='page-link' href='posts.php?mypost&status={$_GET['status']}&page={$i}'>$i</a></li>";
				} else {
...
DELIMETER;
		...
		<td><a href="index.php?delete_admin_post={$row['post_id']}" onClick="javascript: return confirm('Are you sure to delete this post?')">Delete</a></td>
		</tr>
DELIMETER;
	echo $post;
	}
...
function delete_post($id) {
	...
	if(isset($_GET['delete_admin_post'])) {
		set_message("post has been delete");
...
function get_order_option_for_edit($id) {
	$query = query("SELECT * FROM categories");
	confirmQuery($query);
	...
	for ($i=0 ; $i<$num ; $i++){
		$order = $i + 1;
		$selected = $id==$order ? 'selected':'';
		echo "<option val='$order' $selected>$order</option>";
	}
}
...
function get_messages() {
	...
	while ($row = fetch_array($query)) {
		$message = <<< DELIMETER
		<tr><td><strong>{$row['message_title']} mgs.</strong></td><td>{$row['message_content']}</td></tr>
DELIMETER;
		echo $message;
	}
}

function get_messages_for_edit() {
	$query = query("SELECT * FROM messages");
	confirmQuery($query);
	while ($row = fetch_array($query)) {
		$message = <<< DELIMETER
		<tr><td><strong>{$row['message_title']}</strong></td>
		<td><input class="form-control" type="text" name="{$row['message_title']}" value="{$row['message_content']}"></td></tr>
DELIMETER;
		echo $message;
	}
}
...
function edit_message() {
	if(isset($_POST['msg-edit'])) {
		$title = escape_string($_POST['title']);
		$welcome = escape_string($_POST['welcome']);

		// echo "UPDATE messages SET message_content={$title} WHERE message_title='title'";
		$query_1 = query("UPDATE messages SET message_content='{$title}' WHERE message_title='title'");
		confirmQuery($query_1);
		$query_2 = query("UPDATE messages SET message_content='{$welcome}' WHERE message_title='welcome'");
		confirmQuery($query_2);
